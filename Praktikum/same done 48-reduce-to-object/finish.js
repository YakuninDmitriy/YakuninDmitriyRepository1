...
const quantitiesByCategories = (products) => {
    return products.reduce((finObj, el, i, arr) => {
      if(el.category == 'Accessories') {
        finObj.accessories += el.quantity
      }
      else if(el.category == 'Phones') {
        finObj.phones += el.quantity
      }
      else if(el.category == 'Watches') {
        finObj.watches += el.quantity
      }
      return finObj
  }, {
      accessories: null,
